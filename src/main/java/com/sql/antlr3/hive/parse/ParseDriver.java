...
/**
 * 此类为antlr的词法分析部分提供了不区分大小写的令牌检查器的实现。通过在检查词汇规则时将令牌流转换为大写，
 * 此类可确保词汇规则仅需要将令牌与大写字母匹配，而不是将大写和小写字符组合在一起。这纯粹是用于匹配词汇规则。
 * 实际令牌文本的存储方式与用户输入相同，而无需将其实际转换为大写形式。令牌值是由超类ANTLRStringStream的消耗（）函数生成的。
 * LA（）函数是先行功能，仅用于匹配词汇规则。这也意味着该语法仅在从其他工具（例如不具有ANTLRNoCaseStringStream实现的antlrworks）运行的情况下才接受大写的令牌。.
 */
            ...
            if (AllCommentsDeq.size() > 0) {
                ...
                while (firstcomment.Where == null || firstcomment.Where == node.getToken()) {
                    ...
                    if (sb.length() > 0 && sb.charAt(sb.length() - 1) == '\n' && NeedRemoveLastEnter(sb)) {
                        ...
                        sb.append(StringUtils.repeat(" ", ParseDriver.TABSPACE));
    ...
    public boolean CreateFormat(boolean KeyUpper, ASTNode node, StringBuilder sb, int tabLength,
        ...
        switch (node.getType()) {
            ...
            case HiveParser.TOK_SERDEPROPS: {
                List<Node> c = node.getChildren();
                for (Node i : c) {
                    sb.append(StringUtils.repeat(" ", (tabLength + 1) * ParseDriver.TABSPACE));
                    switch (((ASTNode) i).getType()) {
                ...
                for (Node i : c) {
                    switch (((ASTNode) i).getType()) {
                        ...
                        case HiveParser.TOK_TIMESTAMP:
                            sb.append(KeywordFormat(" timestamp", KeyUpper));
                            break;
                        // 注释
...
//                sb.append(StringUtils.repeat(" ", tabLength * FormatDriver.TABSPACE));
            ...
            case HiveParser.TOK_FROM: {
                ...
                sb.append("  ");
                sb.append(KeywordFormat(HiveParser.xlate("KW" + node.getText().substring(3)), KeyUpper));
//                System.out.println(KeywordFormat(HiveParser.xlate("KW" + node.getText().substring(3)), KeyUpper)); //from
//                sb.append(KeywordFormat("\n", KeyUpper));
                sb.append(" ");
                List<Node> c = node.getChildren();
                for (Node i : c) {
